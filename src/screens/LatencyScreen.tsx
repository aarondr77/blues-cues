...
export function LatencyScreen() {
  const { latency, setLatency, setPhase } = useAppStore();
    ...
    <section className="panel">
      ...
      {result && (
        <div className={result.reliable ? 'result' : 'result warning'}>
          ...
          {!result.reliable && (
            <p>
              That is above {MAX_RELIABLE_OFFSET_MS} ms or varies by more than{' '}
              {MAX_RELIABLE_STD_DEV_MS} ms, so timing scores will be unreliable. Close other audio
